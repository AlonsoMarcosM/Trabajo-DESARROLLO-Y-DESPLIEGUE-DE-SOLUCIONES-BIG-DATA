...
AGGREGATIONS_TABLE = f"{CATALOG}.{DATABASE}.gold_customer_aggregations"
PROFILE_TABLE      = f"{CATALOG}.{DATABASE}.gold_customer_profile"

# Joined training table (spine + aggregations + profile)
TRAINING_TABLE     = f"{CATALOG}.{DATABASE}.gold_churn_training_dataset"

catalog            = CATALOG
database           = DATABASE
spine_table        = SPINE_TABLE
aggregations_table = AGGREGATIONS_TABLE
...
DATE_COLUMN         = "usage_event_time"  # temporal key used for train/val/test splits
CUSTOMER_ID_COLUMN  = "customer_id"           # entity key
...
# capped to avoid stale churn patterns degrading performance.
...
project      = "customer_churn_detection"
team         = "ml_engineering"
...
_max_date_raw         = df_raw.agg(
    F.max(F.col(date_column)).alias("max_date")
).collect()[0]["max_date"]

if isinstance(_max_date_raw, str):
    test_end_date = _max_date_raw[:10]          # already "YYYY-MM-DD", trim time if present
else:
    test_end_date = _max_date_raw.strftime("%Y-%m-%d")
...
def apply_class_weights(df):
    ...
    """
    n_total  = df.count()
    n_churn  = df.filter(F.col(label_column) == 1).count()
    n_active = n_total - n_churn

    pct_churn  = 100 * n_churn  / n_total if n_total > 0 else 0.0
    pct_active = 100 * n_active / n_total if n_total > 0 else 0.0

    weight_churn  = n_total / (2.0 * n_churn)  if n_churn  > 0 else 1.0
    weight_active = n_total / (2.0 * n_active) if n_active > 0 else 1.0

    print(f"Total rows for training: {n_total:,}")
    print(f"Churn:  {n_churn:,}  ({pct_churn:.2f}%),  weight = {weight_churn:.2f}")
    print(f"Active: {n_active:,} ({pct_active:.2f}%), weight = {weight_active:.2f}")
    print()

    weighted_df = df.withColumn(
        class_weight_column,
        F.when(F.col(label_column) == 1.0, weight_churn).otherwise(weight_active)
    )
...
# Binary flags stored as integers: treated as boolean features
# Sources: gold_customer_profile
binary_flag_columns = [
    "has_tv_bundle",
    "has_fiber",
    "has_roaming",
...
# Columns that are never included in the feature vector:
# - the label and entity/temporal keys (spine)
# - SCD Type-2 bookkeeping timestamps (profile)
# - raw churn_date (leaks the label)
# - year_month (redundant with label_available_date)
# - signup_date (encoded as tenure_months in feature engineering instead)
# They are excluded here rather than dropped in the pipeline so they remain
# available after transform for joins and traceability downstream.
exclude_columns = [
    label_column,
    "customer_id",
    "usage_event_time",
...
# Stage 1: imputation
#
# Aggregation columns from gold_customer_aggregations:
#   call_minutes, days_payment_late  → LONG, default to 0 on empty windows (no imputation)
#   data_consumed_gb, bill_amount, nps_score, coverage_score, bill_vs_data_ratio
#                                    → DOUBLE, arrive as null on empty windows (median imputation)
agg_zero_columns = [c for c in numeric_columns if c in {"call_minutes", "days_payment_late"}]
agg_null_columns = [c for c in numeric_columns if c in {
    "data_consumed_gb", "bill_amount", "nps_score", "coverage_score", "bill_vs_data_ratio"
}]
profile_numeric_columns = [
    c for c in numeric_columns
    if c not in agg_zero_columns and c not in agg_null_columns
]
...
#
# COALESCE handles nulls inline: a missing flag is treated as disabled (0.0)
...
#
# Only features computable from the current customer snapshot at inference time.
# Features requiring historical aggregations are sourced from the feature store
...
feature_engineering_statement = (
    ...
    "CAST((days_payment_late > 0) AS INT) AS has_late_payment, "
    # NPS detractor flag: scores 0-6 are detractors by standard NPS definition
    "CAST((nps_score <= 6.0) AS INT) AS is_nps_detractor, "
    # Low coverage flag: below 50% signal quality
    "CAST((coverage_score < 0.5) AS DOUBLE) AS is_low_coverage, "
    # Log-transform skewed monetary feature to reduce leverage of outliers
    "LOG(bill_amount + 1) AS bill_amount_log "
    "FROM __THIS__"
)
engineered_columns = [
    "tenure_months",
...
# Categorical columns sourced from gold_customer_profile:
#   gender, contract_type, region, region_type, tariff_plan,
#   device_type, acquisition_channel, payment_method,
#   age_group, contract_risk_group
...
def fig_lr_coefficients(coef_array, feature_names, title):
    ...
    Red means positive coefficient (push toward churn) and
    blue means negative (push toward active/retained).
    """
    ...
    ax.legend(
        handles=[
            Patch(color=color_positive_coef, label="Indicative of churn"),
            Patch(color=color_negative_coef, label="Indicative of retention"),
        ],
        loc="lower right",
        fontsize=8
    )
...
# Hard limit for ".toPandas()" conversions. Bringing massive distributed datasets
# into the cluster driver's local memory for processing can cause severe OOM crashes.
...
def to_pandas_predictions(predictions):
    """
    Convert prediction columns to a local Pandas DataFrame.

    Extracts the churn probability from the probability vector at index 1
    ...
    return (
        ...
        .assign(
            **{
                prob_churn_column: lambda df: df[probability_column].apply(
