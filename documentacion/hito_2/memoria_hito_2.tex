...
A continuación se describe cada capa por separado para que quede claro qué responsabilidad tiene, qué tablas materializa y qué datos contiene cada salida. Los recuentos indicados corresponden a la carga histórica usada en Hito 2: \texttt{context/customers.csv} y \texttt{events/*} para 2023--2024. Los ficheros de 2025 se mantienen en \texttt{source\_buffer/*} como producción simulada con deriva y no entran automáticamente en estas tablas si no se amplía la ingesta.
...
\begin{itemize}
    \item Separación estricta entre código versionable y datos generados. Los directorios \texttt{context/}, \texttt{events/}, \texttt{source\_buffer/} y los logs del generador se excluyen del bundle para que \texttt{validate} y \texttt{deploy} no intenten sincronizar artefactos masivos.
